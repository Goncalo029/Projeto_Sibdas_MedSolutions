...
try {
    ...
    // vai buscar o fornecedor que se quer apagar
    $id = $_GET['id'] ?? 0;
    $stmt = $pdo->prepare("SELECT * FROM fornecedores WHERE id = ? LIMIT 1");
    $stmt->execute([$id]);
    $fornecedor = $stmt->fetch(PDO::FETCH_ASSOC);

    // se nao existir avisa e volta para a lista
    if (!$fornecedor) {
        $_SESSION['error_message'] = 'Fornecedor não encontrado!';
        echo '<script>window.location.href = "lista.php";</script>';
        exit;
    }
} catch (Exception $e) {
    $_SESSION['error_message'] = 'Erro ao carregar fornecedor: ' . $e->getMessage();
    echo '<script>window.location.href = "lista.php";</script>';
    exit;
}

$page_title = 'Fornecedores - Apagar';
...
<div class="mhs-page-header mhs-page-header--dashboard">
  <div>
    ...
    <h1 class="mhs-page-title">Fornecedores - Apagar</h1>
  </div>
  ...
  <div class="card-body">
    <div class="alert alert-danger mb-4">
      ...
      Tem a certeza que pretende apagar este fornecedor. Esta ação não pode ser revertida.
    </div>
    <div class="row g-3 mb-4">
      <div class="col-md-6">
        <label class="form-label fw-semibold">Registo selecionado</label>
        <input class="form-control" value="fornecedor selecionado" readonly />
      </div>
      <div class="col-md-6">
    ...
    <div class="d-flex gap-2 mt-3">
      <form method="POST" action="confirmar_apagar.php" style="flex: 0 0 auto;">
        <input type="hidden" name="id_enc" value="<?php echo htmlspecialchars($fornecedor['id']); ?>">
