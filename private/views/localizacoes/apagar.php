...
try {
    ...
    // vai buscar a localizacao que se quer apagar
    $id = $_GET['id'] ?? 0;
    $stmt = $pdo->prepare("SELECT * FROM localizacoes WHERE id = ? LIMIT 1");
    $stmt->execute([$id]);
    $localizacao = $stmt->fetch(PDO::FETCH_ASSOC);

    // se nao existir avisa e volta para a lista
    if (!$localizacao) {
        $_SESSION['error_message'] = 'Localização não encontrada!';
        echo '<script>window.location.href = "lista.php";</script>';
        exit;
    }
} catch (Exception $e) {
    $_SESSION['error_message'] = 'Erro ao carregar localização: ' . $e->getMessage();
    echo '<script>window.location.href = "lista.php";</script>';
    exit;
}

$page_title = 'Localizações - Apagar';
...
<div class="mhs-page-header mhs-page-header--dashboard">
  <div>
    ...
    <h1 class="mhs-page-title">Localizações - Apagar</h1>
  </div>
  ...
  <div class="card-body">
    <div class="alert alert-danger mb-4">
      ...
      Tem a certeza que pretende apagar este localização. Esta ação não pode ser revertida.
    </div>
    <div class="row g-3 mb-4">
      <div class="col-md-6">
        <label class="form-label fw-semibold">Registo selecionado</label>
        <input class="form-control" value="localização selecionado" readonly />
      </div>
      <div class="col-md-6">
    ...
    <div class="d-flex gap-2 mt-3">
      <form method="POST" action="confirmar_apagar.php" style="flex: 0 0 auto;">
        <input type="hidden" name="id_enc" value="<?php echo htmlspecialchars($localizacao['id']); ?>">
